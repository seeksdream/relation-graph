...
export class RelationGraphWith7Event extends RelationGraphWith6Effect {
  ...
  onNodeClick(node:RGNode, e:RGUserEvent) {
    ...
    devLog('[node]node click', node.text);
  ...
  onLineClick(line:RGLine, link:RGLink, e:RGUserEvent) {
  ...
  startCreatingNodePlot(e:RGUserEvent, setting:CreatingNodePlotOptions) {
    this.options.newNodeTemplate = json2Node(JSON.parse(JSON.stringify(newNodeTemplate)));
  ...
  onCanvasDragStart(e:RGUserEvent) {
    ...
    if (isSupportTouch(e)) {
      ...
      let baseZoom = 1;
      draggingCallback = (x:number, y:number, basePosition:RGPosition, baseEventPosition:RGPosition, e:RGUserEvent) => {
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
    ...
    if (this.options.disableDragCanvas || e.shiftKey) {
